        ...
        for i, statement in enumerate(sql_statements):
            ...
            if statement and not statement.startswith('--'):
                try:
                    ...
                    # 跳过触发器可能已存在的错误
                    if "already exists" in str(e) and "TRIGGER" in str(e):
                        print(f"  执行语句 {i+1}/{len(sql_statements)}: 跳过（触发器已存在）")
                    elif "already exists" in str(e) and "INDEX" in str(e):
                        print(f"  执行语句 {i+1}/{len(sql_statements)}: 跳过（索引已存在）")
                    elif "already exists" in str(e) and "TABLE" in str(e):
                        print(f"  执行语句 {i+1}/{len(sql_statements)}: 跳过（表已存在）")
                    elif "already exists" in str(e) and "COLUMN" in str(e):
                        print(f"  执行语句 {i+1}/{len(sql_statements)}: 跳过（字段已存在）")
...
def main():
    """主函数"""
    print("=" * 60)
    print("用户系统数据库迁移工具 (自动执行)")
    print("=" * 60)
    
    # 自动执行迁移，不需要用户确认
    print("\n⚠️  正在执行数据库迁移，可能会修改现有表结构...")
