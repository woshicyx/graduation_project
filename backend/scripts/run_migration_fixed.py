...
def main():
    ...
    # 询问用户确认
    print("\n⚠️  即将执行数据库迁移，可能会修改现有表结构。")
    print("注意: 此操作需要在命令行中运行以进行用户输入确认")
    print("\n请在命令行中运行此脚本以执行迁移:")
    print("python backend/scripts/run_migration_fixed.py")
    
    # 这里我们不等待用户输入，因为工具环境无法处理交互式输入
    print("\n由于环境限制，无法在此处执行交互式迁移。")
    print("请手动运行迁移脚本:")
    print("1. 打开命令行")
    print("2. cd d:\\projects\\MovieAI")
    print("3. python backend\\scripts\\run_migration_fixed.py")
